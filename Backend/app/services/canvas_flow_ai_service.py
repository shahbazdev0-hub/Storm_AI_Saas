...
#      """Get services from Canvas Flow or return defaults"""
#     try:
...
#         # Try to extract from Canvas Flows first
#         for flow in flows:
#             nodes = flow.get('nodes', [])
#             for node in nodes:
#                 title = node.get('data', {}).get('title', '')
#                 if title and title not in ['Begin', 'Welcome Node', 'Services', 'Start', 'End'] and title.lower() not in seen_names:
#                     services.append({
#                         "name": title,
...
#         if len(services) == 0:
#             logger.info(f"Using default services for company {company_id}")
#             services = [
#                 {"name": "Home Cleaning", "description": "Professional residential cleaning services"},
#                 {"name": "Deep Cleaning", "description": "Thorough deep cleaning for your entire space"},
#                 {"name": "Plumbing Repair", "description": "Expert plumbing repair and maintenance"},
#                 {"name": "Electrical Work", "description": "Licensed electrical installation and repairs"},
#                 {"name": "HVAC Service", "description": "Heating, ventilation, and air conditioning services"}
#             ]
#             logger.info(f"Loaded {len(services)} default services")
#         else:
#             logger.info(f"Loaded {len(services)} services from Canvas Flows")
        
#         return services[:5]  # Return max 5 services
        
#     except Exception as e:
#         logger.error(f"Error loading Canvas services: {e}")
#         # Return defaults on error
#         return [
#             {"name": "General Service", "description": "General service assistance"},
#             {"name": "Home Cleaning", "description": "Professional home cleaning"},
#             {"name": "Maintenance", "description": "General maintenance services"}
#         ]



# backend/app/services/canvas_flow_ai_service.py - COMPLETE FIXED VERSION
import logging
import openai
from typing import Dict, List, Any, Optional
from datetime import datetime, timedelta
...
class CanvasFlowAIService:
    def __init__(self, db: AsyncIOMotorDatabase):
        ...
        api_key = "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"
    ...
    def _analyze_intent(self, message: str) -> str:
        ...
        if any(word in message_lower for word in ["services", "what do you offer", "provide", "what can you do"]):
